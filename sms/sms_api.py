...
def sendSMS_Worthy_Index_Completed(mobile_number,info_dict):
	test = RestAPI()
	accountSid = data_config.accountSid
	accountToken = data_config.accountToken
	appId = "8cae77cfbad34fae9702f22b6d602908"
	isUseJson = True
	toNumber = mobile_number
	templateId="7405"
	param="%s,%s" %(info_dict['num'],10)
	# print param
	# exit()
	ret = test.templateSMS(accountSid,accountToken,appId,toNumber,templateId,param,isUseJson)
	print ret
	rc = -1
	try:
		sdict = json.loads(ret)
		if sdict['resp']['respCode'] == '000000':
			rc = 0
		cdate = sdict['resp']['templateSMS']['createDate']
		print cdate
	except:
		pass
	return rc
...
if __name__ == "__main__":
	sendSMS_Worthy_Index_Completed('18501070400',{})
